...
from capture_training_data import TRAININGDATA
...
"""
username='root'
password="Mydorito048187686332"
host="localhost"
db="FaceRecognitionMusaib"
port="3306"
...
class Student:
    def __init__(self,root):
        ...
        self.root.title("RIMT-IITD Attendance System | Student Registration")
        ...
        self.var_can_save = StringVar()
        self.temp_images_dir = "./temp"
        self.images_dir = "./data_images/train"
    ...
    # This part is image labels setting start 
        ...
        #title section
        title_lb1 = Label(bg_img,text="Register Student | RIMT-IITD Attendance Managment System",font=("verdana",30,"bold"),bg="white",fg="navyblue")
        ...
        # Left Label Frame 
        left_frame = LabelFrame(main_frame,bd=2,bg="white",relief=RIDGE,text="Student Details",font=("verdana",8,"normal"),fg="navyblue")
        ...
        dep_combo["values"]=("Select Department","B.Tech","M.Tech","B.Sc","M.Sc", "B.Com", "M.Com", "Diploma", "BA", "MA","BCA", "B.Pharma", "M.Pharma", "Others")
        dep_combo.current(0)
        ...
        cou_combo["values"]=("Select Course","CSE","Electrical","Electronics","Mechanical", "Agriculture","Hospitality","Pharmacy","Journalism","Others")
        cou_combo.current(0)
        ...
        #label Year
        year_label=Label(current_course_frame,text="Year of Entry",font=("verdana",12,"bold"),bg="white",fg="navyblue")
        ...
        class_Student_frame = LabelFrame(left_frame,bd=2,bg="white",relief=RIDGE,text="Class Student Information",font=("verdana",8,"normal"),fg="navyblue")
        ...
        #Roll No
        student_roll_label = Label(class_Student_frame,text="Roll/Emp-No:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        ...
        #take photo button
        self.take_photo_btn=Button(class_Student_frame,command=self.generate_dataset,text="Take Pic",width=9,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        self.take_photo_btn.grid(row=5,column=0,padx=5,pady=10,sticky=W)
        ...
        #save button
        self.save_btn=Button(btn_frame,command=self.add_data,text="Save",width=7,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        self.save_btn.grid(row=0,column=0,padx=5,pady=10,sticky=W)

        #update button
        self.update_btn=Button(btn_frame,command=self.update_data,text="Update",width=7,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        self.update_btn.grid(row=0,column=1,padx=5,pady=8,sticky=W)

        #delete button
        self.del_btn=Button(btn_frame,command=self.delete_data,text="Delete",width=7,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        self.del_btn.grid(row=0,column=2,padx=5,pady=10,sticky=W)

        #reset button
        self.reset_btn=Button(btn_frame,command=self.reset_data,text="Reset",width=7,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        self.reset_btn.grid(row=0,column=3,padx=5,pady=10,sticky=W)


        #update photo button
        self.update_photo_btn=Button(btn_frame,text="Update Pic",width=9,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        self.update_photo_btn.grid(row=0,column=5,padx=5,pady=10,sticky=W)
        ...
        #create table 
        self.student_table = ttk.Treeview(table_frame,column=("ID","Roll-No", "Name","Dep","Course","Year","Gender","DOB","Mob-No","Address","Email","Photo"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
...
# ==================Function Decleration==============================
    def add_data(self):
        ...
        if self.var_dep.get()=="Select Department" or self.var_course.get=="Select Course" or self.var_year.get()=="Select Year" or self.var_std_name.get()=="" or self.var_roll.get()=="" or self.var_gender.get()=="" or self.var_dob.get()=="" or self.var_email.get()=="" or self.var_mob.get()=="" or self.var_address.get()=="":
            messagebox.showerror("Error","Please Fill All Fields are Required!",parent=self.root)
        elif self.var_can_save.get() == "NO" or self.var_can_save.get() == "":
            messagebox.showerror("Error","Make sure you take dataset of the candidate!",parent=self.root)
        else:
            try:
                id_new = int(self.var_std_id.get())+1
                conn = mysql.connector.connect(username=username, password=password,host=host,database=db,port=port)
                ...
                src= self.temp_images_dir+"/"+str(id_new)
                dst= self.images_dir
                #print(src, dst)
                shutil.move(src, dst)
                conn.commit()
    ...
    def fetch_data(self):
        ...
        mycursor.execute("select Student_ID,Roll_No,Name,Department,Course,Year,Gender,DOB ,Mobile_No,Address,Email,PhotoSample  from student")
    ...
    def get_cursor(self,event=""):
        ...
        #enable update, delete, update pic
        self.update_buttons_statechanger(NORMAL)
        self.new_buttons_statechanger(DISABLED)


    # ========================================Update Function==========================
    def update_data(self):
        if self.var_dep.get()=="Select Department" or self.var_course.get=="Select Course" or self.var_year.get()=="Select Year" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_roll.get()=="" or self.var_gender.get()=="" or self.var_dob.get()=="" or self.var_email.get()=="" or self.var_mob.get()=="" or self.var_address.get()=="":
            messagebox.showerror("Error","Please Fill All Fields are Required!",parent=self.root)
        else:
            try:
                ...
                if Update > 0:
                    conn = mysql.connector.connect(username=username, password=password,host=host,database=db,port=port)
                    mycursor = conn.cursor()
                    mycursor.execute("update student set Name=%s,Department=%s,Course=%s,Year=%s,Gender=%s,DOB=%s,Mobile_No=%s,Address=%s,Roll_No=%s,Email=%s,PhotoSample=%s where Student_ID=%s",( 
                    self.var_std_name.get(),
                    self.var_dep.get(),
                    self.var_course.get(),
                    self.var_year.get(),
    ...
    def reset_data(self):
        ...
        #print(myresult)
        if myresult[0]== None:
            self.var_std_id.set("0")
        else:
            self.var_std_id.set(myresult[0]),
        ...
        self.var_email.set(""),
        self.var_radio1.set("")
        self.var_can_save.set("")

        self.update_buttons_statechanger(DISABLED)
        self.new_buttons_statechanger(NORMAL)

    def update_buttons_statechanger(self, state):
        self.update_btn.config(state=state)
        self.del_btn.config(state=state)
        self.update_photo_btn.config(state=state)            

    def new_buttons_statechanger(self, state):
        self.save_btn.config(state=state)
        self.take_photo_btn.config(state=state)
    ...
    def search_data(self):
        ...
        if self.var_search.get()=="" or self.var_searchTX.get()=="Select":
            ...
            try:
                ...
                sql = "SELECT Student_ID,Roll_No,Name,Department,Course,Year,Gender,DOB ,Mobile_No,Address,Email,PhotoSample FROM student where Roll_No = '" +str(self.var_search.get()) + "'" 
                #print(sql)
    ...
    def generate_dataset(self):
        id_new = int(self.var_std_id.get())+1
        obj = TRAININGDATA(self.temp_images_dir+"/"+str(id_new),self.images_dir+"/"+str(id_new))
        ret = obj.capture_images_from_camera()
        
        if ret == "COMPLETE":
            self.var_can_save.set("YES")
        else:
            self.var_can_save.set("NO")
